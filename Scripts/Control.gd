...
func _ready():
	...
	cp.title = "PROPERTY EARNING RATE"
	cp.x_label = "Time (s)"
	cp.y_label = "$/s"
	cp.x_scale = 5
	cp.y_scale = 10
	...
	f1 = Function.new(
		x, y, "PER", # This will create a function with x and y values taken by the Arrays 
...
func setNewPoint(time:float,earn:float):
	f1.add_point(time, earn)
	#f1.remove_point(0)
	chart.queue_redraw()
